...
*** TODO Get handle-transform-param-change to only submit params for the transform that has been used
*** TODO Trigger a picture reload if user checks/unchecks an optional param #6
*** TODO Add a value label next to the slider bar so that users know what actual values they are setting params to
*** TODO Make sure that Gaussian Blur filter works and that it loads in all of the option-groups
*** TODO Test that adding a transformation to the pipeline with a non default source works correctly
*** TODO Write code that will reperform all of the transforms if a new source is selected
*** TODO Get Select from uploaded images to remember the images uploaded that session and allow for selection
...
*** TODO Add listener to the close button that removes the frame from the pipeline.
